...
const STABLE_SWAP_ADDRESS = process.env.STABLE_SWAP_ADDRESS;
const STABLE_SWAP_ADDRESS_ETHERLINK = process.env.STABLE_SWAP_ADDRESS_ETHERLINK;
if (!PRIVATE_KEY) {
...
const signerAccount = (0, accounts_1.privateKeyToAccount)(PRIVATE_KEY);
const CHAINS = {
    base_sepolia: {
        key: "base_sepolia",
        name: "Base Sepolia",
...
const DEFAULT_CHAIN = (() => {
    if (!process.env.DEFAULT_CHAIN)
        return "etherlink_shadownet";
    try {
        return parseChainKey(process.env.DEFAULT_CHAIN) ?? "etherlink_shadownet";
    }
    catch (error) {
        console.warn(`WARN: DEFAULT_CHAIN is invalid ("${process.env.DEFAULT_CHAIN}"). Falling back to etherlink_shadownet.`);
        return "etherlink_shadownet";
    }
})();
const publicClients = {};
for (const chain of Object.values(CHAINS)) {
    if (chain.rpcUrl) {
        publicClients[chain.key] = (0, viem_1.createPublicClient)({
...
async function signPaymasterData(params) {
    ...
    const hash = (0, viem_1.keccak256)((0, viem_1.encodeAbiParameters)((0, viem_1.parseAbiParameters)("address, address, uint256, uint256, bool"), [
        payerAddress,
        tokenAddress,
        BigInt(validUntil),
        BigInt(validAfter),
...
function requireAddress(addr, field) {
...
function resolveChainKey(input) {
    const parsed = parseChainKey(input);
    return parsed ?? DEFAULT_CHAIN;
}
function getChainContext(req) {
    const chainKey = resolveChainKey(req.query.chain ??
        req.query.chainId ??
        req.body?.chain ??
        req.body?.chainId ??
        req.headers["x-chain"]);
